...
export default function RegisterPage() {
  ...
  const [codigoVerificacionModo, setCodigoVerificacionModo] = useState(false)
  ...
  const handlePasswordChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await fetch('/api/comprobarRegistro', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ correo: email }),  // usa "correo"
      });

      if (!response.ok) {
        const errorData = await response.json();
        alert(`Error: ${errorData.message || 'No se pudo registrar'}`);
        setMensajeError('Ya existe una cuenta con este correo')
        setIsLoading(false);
        return;
      }
      setMensajeError('')
      ...
      setCodigoVerificacionModo(true)
    ...
    try {
      ...
      if (data.resultado) {
        alert('Registro completado correctamente');
          try {
          const resMeterDatos = await fetch('/api/registroAnadir', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ nombre: name, correo: email, contrasena: password }),
          });
          if(resMeterDatos.ok){
            console.log(resMeterDatos)
      ...
      setMensajeError('Código invalido')
      console.log(error)
    } finally {
      setIsLoading(false);
    }
          ...
          <form onSubmit={handleSubmit} className={styles.authForm}>
            ...
            <div className={styles.termsContainer}>
              <label className={styles.checkboxContainer}>
                ...
                <p style={{color: 'red', fontWeight: 'bolder'}}>{mensajeError}</p>
            </div>
...
*/}
          ...
          <form onSubmit={handleVerifyCode} className={styles.authForm}>
            ...
            <button type="submit" disabled={isLoading} className={styles.authButton}>Verificar código</button>
            <p>{mensajeError}</p>
          </form>
        )}
      </div>
